...
function MyTorpedo(scene, submarine, target) {
	...
	this.t = 0.0;
	this.dist = Math.sqrt(((this.sub_x-this.target.x), 2) +
								Math.pow((this.sub_y-this.target.y), 2) +
								Math.pow((this.sub_z-this.target.z), 2));
...
MyTorpedo.prototype.update = function(elapsed){
	...
	var p4x = 7.5-this.target.x; var p4y = this.target.y-0.5; var p4z = 8-this.target.z;
